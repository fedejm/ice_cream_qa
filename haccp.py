...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""QA App – Multi‑section Streamlit app for Lush Gelato

This is a single‑file app with a sidebar navigation (like your recipes app).
Sections:
  • Login (stores user in st.session_state)
  • Mix Prep Log (auto‑assign earliest ingredient expiration)
  • Batch Freezing Log (operator + mixer + second tester)
  • Admin: Employees (maintain employees.csv)
  • Dashboard (recent activity + downloads)

Data files (created automatically under ./data):
  data/employees.csv
  data/mix_prep_log.csv
  data/batch_freezing_log.csv
...
# App config
# -----------------------------
st.set_page_config(page_title="QA App – Lush Gelato", page_icon="🍦", layout="wide")

DATA_DIR = "data"
EMPLOYEE_CSV = os.path.join(DATA_DIR, "employees.csv")
MIX_PREP_LOG = os.path.join(DATA_DIR, "mix_prep_log.csv")
FREEZE_LOG = os.path.join(DATA_DIR, "batch_freezing_log.csv")
...
def load_employees() -> pd.DataFrame:
    ...
    if os.path.exists(EMPLOYEE_CSV):
        try:
            ...
            df = pd.DataFrame(columns=["full_name", "is_active"])
    else:
        df = pd.DataFrame(columns=["full_name", "is_active"])
    # Normalize
...
def save_employees(df: pd.DataFrame):
    ...
    keep = [c for c in ["full_name", "is_active"] if c in df.columns]
    df = df[keep]
    df.to_csv(EMPLOYEE_CSV, index=False)


def employee_names(active_only: bool = True) -> List[str]:
    df = load_employees()
    if active_only and "is_active" in df.columns:
        df = df[(df["is_active"].astype(str).str.lower().isin(["1", "true", "yes"])) | (~df["is_active"].notna())]
    names = (
        df.get("full_name", pd.Series(dtype=str))
        .dropna().astype(str).str.strip().sort_values().unique().tolist()
    )
    # fallback if empty
    return names or ["Alice Martinez", "Ben Kim", "Carla Rossi", "Diego Alvarez", "Eva Chen"]


def append_csv(path: str, record: Dict):
    ensure_data_dir()
...
def current_user() -> str | None:
    return st.session_state.get("user_full_name")


def login_widget():
    st.subheader("Login")
    names = employee_names(active_only=True)
    c1, c2 = st.columns([1, 1])
    with c1:
        name = st.selectbox("Select your name", names, key="login_select_name")
    with c2:
        # simple optional PIN field (not secure – placeholder until real auth)
        pin = st.text_input("Optional PIN (demo)", type="password", key="login_pin")
...
# -----------------------------
# Section: Mix Prep Log
# -----------------------------

def section_mix_prep():
    st.header("🥣 Mix Preparation Log")
    if not require_login():
        return

    with st.form("mixprep_form"):
        col1, col2 = st.columns(2)
        with col1:
            production_date = st.date_input("Production Date", value=date.today(), key="mp_prod_date")
            batch_id = st.text_input("Batch ID / Code", placeholder="e.g., 2025-09-22-VAN-001", key="mp_batch_id")
        with col2:
            # Ingredient expirations
            milk_exp = st.date_input("Milk expiration date", key="mp_milk_exp")
            cream_exp = st.date_input("Cream expiration date", key="mp_cream_exp")
            yolks_exp = st.date_input("Egg yolks expiration date", key="mp_yolk_exp")

        taste_tested = st.radio("Taste test completed for finished mix?", ["Yes", "No"], horizontal=True, key="mp_taste")
        notes = st.text_area("Notes (optional)", key="mp_notes")

        submitted = st.form_submit_button("Save Mix Prep Log")

    if submitted:
        # earliest expiration
        earliest_exp = min(milk_exp, cream_exp, yolks_exp)
        errs = []
        if not batch_id:
            errs.append("Please enter a Batch ID / Code.")
        if earliest_exp < production_date:
            errs.append("Earliest expiration cannot be earlier than the production date.")
        if errs:
            for e in errs:
                st.error(e)
            return
        record = {
            ...
            "milk_exp": str(milk_exp),
            "cream_exp": str(cream_exp),
            "yolks_exp": str(yolks_exp),
            "assigned_batch_exp": str(earliest_exp),
            "mixer": current_user(),
            "mixer_taste_tested": taste_tested,
            "notes": notes.strip(),
        }
        append_csv(MIX_PREP_LOG, record)
        st.success(f"Saved. Assigned batch expiration = {earliest_exp}")

    # Recent entries
    if os.path.exists(MIX_PREP_LOG):
        df = pd.read_csv(MIX_PREP_LOG)
        if not df.empty:
            st.subheader("Recent Mix Prep Entries")
            df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
            st.dataframe(df.sort_values("timestamp", ascending=False).head(25), use_container_width=True)
            st.download_button(
                "⬇️ Download Mix Prep Log (CSV)",
                data=df.to_csv(index=False).encode("utf-8"),
                file_name="mix_prep_log.csv",
                mime="text/csv",
            )


# -----------------------------
# Section: Batch Freezing Log
# -----------------------------

def section_batch_freeze():
    st.header("🧊 Batch Freezing Log")
    if not require_login():
        return
    ...
    with st.form("freeze_form"):
        col1, col2 = st.columns(2)
        with col1:
            batch_date = st.date_input("Batch Date", value=date.today(), key="bf_date")
            mix_exp_date = st.date_input("Mix Expiration Date", key="bf_mix_exp")
        with col2:
            batch_id = st.text_input("Batch ID / Code", placeholder="e.g., 2025-09-22-VAN-003", key="bf_batch_id")

        mixer_name = st.selectbox("Mixer (who made the mix)", names, index=0, key="bf_mixer")

        second_choices = names[:]  # you can filter out current user if you want
        second_tester_name = st.selectbox("Second Tester", second_choices, index=min(1, len(second_choices)-1) if len(second_choices) > 1 else 0, key="bf_second")

        st.markdown("### Taste Test Confirmation")
        t1, t2 = st.columns(2)
        with t1:
            mixer_taste_tested = st.radio("Mixer taste‑tested finished product?", ("Yes", "No"), horizontal=True, key="bf_mixer_tt")
        with t2:
            second_tester_taste_tested = st.radio("Second tester confirmed taste?", ("Yes", "No"), horizontal=True, key="bf_second_tt")

        notes = st.text_area("Notes (optional)", key="bf_notes")
    ...
    if submitted:
        ...
        if not batch_id:
            errs.append("Please enter a Batch ID / Code.")
        if mix_exp_date < batch_date:
            errs.append("Mix expiration date cannot be earlier than the batch date.")
        if errs:
            for e in errs:
                st.error(e)
            return
        ...
        append_csv(FREEZE_LOG, record)
        st.success("Saved batch freezing log ✅")

    # Recent entries
    if os.path.exists(FREEZE_LOG):
        df = pd.read_csv(FREEZE_LOG)
        if not df.empty:
            st.subheader("Recent Freezing Entries")
            df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
            st.dataframe(df.sort_values("timestamp", ascending=False).head(25), use_container_width=True)
            st.download_button(
                "⬇️ Download Batch Freezing Log (CSV)",
                data=df.to_csv(index=False).encode("utf-8"),
                file_name="batch_freezing_log.csv",
                mime="text/csv",
            )
...
def section_admin_employees():
    st.header("🛠️ Admin – Employees")
    if not require_login():
        return
    ...
    with st.form("add_emp_form"):
        ...
        with c1:
            full_name = st.text_input("Full name", key="emp_full_name")
        with c2:
    ...
    if submitted:
        if not full_name.strip():
            ...
            if "is_active" not in df.columns:
                df["is_active"] = True
            # upsert by name
            mask = df["full_name"].astype(str).str.strip().str.lower() == full_name.strip().lower()
            if mask.any():
                df.loc[mask, "is_active"] = is_active
            else:
                df = pd.concat([df, pd.DataFrame([{"full_name": full_name.strip(), "is_active": is_active}])], ignore_index=True)
            save_employees(df)
            st.success("Employee saved.")

    st.subheader("Directory")
    if not df.empty:
        st.dataframe(df.sort_values(["is_active", "full_name"], ascending=[False, True]), use_container_width=True)
        st.download_button(
...
def section_dashboard():
    ...
    if os.path.exists(MIX_PREP_LOG):
        dfm = pd.read_csv(MIX_PREP_LOG)
        if not dfm.empty:
            total_mix = len(dfm)
            try:
                dfm["assigned_batch_exp"] = pd.to_datetime(dfm["assigned_batch_exp"], errors="coerce")
                upcoming = dfm[dfm["assigned_batch_exp"] >= pd.Timestamp.today()].sort_values("assigned_batch_exp").head(1)
                if not upcoming.empty:
                    upcoming_exp = upcoming.iloc[0]["assigned_batch_exp"].date().isoformat()
            except Exception:
                pass
    if os.path.exists(FREEZE_LOG):
        dff = pd.read_csv(FREEZE_LOG)
        if not dff.empty:
            total_freeze = len(dff)

    c1.metric("Mixes logged", total_mix)
    c2.metric("Batches frozen", total_freeze)
    c3.metric("Next batch exp.", upcoming_exp)

    st.divider()
    colL, colR = st.columns(2)
    with colL:
        st.subheader("Latest Mix Prep")
        if os.path.exists(MIX_PREP_LOG):
            dfm = pd.read_csv(MIX_PREP_LOG)
            if not dfm.empty:
                dfm["timestamp"] = pd.to_datetime(dfm["timestamp"], errors="coerce")
                st.dataframe(dfm.sort_values("timestamp", ascending=False).head(10), use_container_width=True)
    with colR:
        st.subheader("Latest Freezing Logs")
        if os.path.exists(FREEZE_LOG):
            dff = pd.read_csv(FREEZE_LOG)
            if not dff.empty:
                dff["timestamp"] = pd.to_datetime(dff["timestamp"], errors="coerce")
                st.dataframe(dff.sort_values("timestamp", ascending=False).head(10), use_container_width=True)
...
def main():
    ...
    if nav == "Login":
        ...
        section_dashboard()
    elif nav == "Mix Prep Log":
        section_mix_prep()
    elif nav == "Batch Freezing Log":
        section_batch_freeze()
    elif nav == "Admin: Employees":
        section_admin_employees()
